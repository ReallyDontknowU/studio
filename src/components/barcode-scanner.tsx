...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({
  ...
  }, [isActive, onScanSuccess, cleanupCamera, onScanError, captureFrame, isScanning]); // Add relevant dependencies
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      // Start the scanning loop AFTER everything is initialized
      setIsScanning(true);
      runScanLoop();
  ...
  }, [toast, onScanError, cleanupCamera, runScanLoop]);
  ...
  return (
    <div className={`flex flex-col items-center gap-4 w-full max-w-xs ${disabled && !isStarting ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      <div className={`w-full aspect-[3/4] border rounded-lg overflow-hidden shadow-md bg-muted relative ${(!isStarting && !isActive && !capturedImageUri) ? 'hidden' : 'block'}`}>
        <video
          ref={videoRef}
